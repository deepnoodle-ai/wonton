...
// Length sets how many characters the rainbow spans.
// If not set, defaults to the text length.
func (r *RainbowAnimation) Length(length int) *RainbowAnimation {
	r.length = length
	return r
}
...
// Amplitude sets the wave amplitude.
func (w *WaveAnimation) Amplitude(amplitude float64) *WaveAnimation {
...
type SlideAnimation struct {
	...
	reversed       bool // True = right to left, false = left to right
}
...
//	Text("Loading").Animate(Slide(2, gray, white).Reverse())
...
// Reverse sets the slide to move from right to left.
func (s *SlideAnimation) Reverse() *SlideAnimation {
	s.reversed = true
	return s
}

// Width sets the width of the highlight in characters.
func (s *SlideAnimation) Width(width int) *SlideAnimation {
	s.width = width
	return s
}
...
//	Text("Stars").Animate(Sparkle(3, gray, white).Density(5))
...
// Density sets how many sparkles appear (1-10 recommended, higher = more sparkles).
func (s *SparkleAnimation) Density(density int) *SparkleAnimation {
	s.density = density
	return s
}
...
//	Text("Hello, World!").Animate(Typewriter(4, white, green).Loop(true))
...
// Loop sets whether the animation should restart after fully revealed.
func (t *TypewriterAnimation) Loop(loop bool) *TypewriterAnimation {
	t.loop = loop
	return t
}

// HoldFrames sets how many frames to hold before looping (if loop is true).
func (t *TypewriterAnimation) HoldFrames(frames int) *TypewriterAnimation {
	t.holdFrames = frames
	return t
}
...
//	Text("SYSTEM ERROR").Animate(Glitch(2, gray, red).Intensity(8))
...
// Intensity sets how often glitches occur (1-10, higher = more glitches).
func (g *GlitchAnimation) Intensity(intensity int) *GlitchAnimation {
