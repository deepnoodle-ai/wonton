...
| cli | Commands, flags, config | Stable | @curtis |`
	view := Markdown(content, &scrollY).MaxWidth(40).Height(10)
	screen := SprintScreen(view, WithWidth(80), WithHeight(10))
	termtest.AssertScreen(t, screen)
}
...
- List items also fill the available width when no max width is configured`
	...
	screen := SprintScreen(view, WithWidth(120), WithHeight(8))
	termtest.AssertScreen(t, screen)
}
...
func TestGolden_Markdown_ExplicitMaxWidth(t *testing.T) {
	...
	screen := SprintScreen(view, WithWidth(80), WithHeight(5))
	termtest.AssertScreen(t, screen)
}
...
func TestGolden_Markdown_HardLineBreakNoMaxWidth(t *testing.T) {
	...
	screen := SprintScreen(view, WithWidth(60), WithHeight(5))
	termtest.AssertScreen(t, screen)
}
...
func TestGolden_Markdown_HorizontalRuleFillsWidth(t *testing.T) {
	...
	content := "Above\n\n---\n\nBelow"
	view := Markdown(content, &scrollY).Height(5)
	screen := SprintScreen(view, WithWidth(100), WithHeight(5))
	termtest.AssertScreen(t, screen)
}
