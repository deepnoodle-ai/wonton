...
func main() {
	...
	debugLogger := slog.New(gooeyslog.NewHandler(os.Stderr, &gooeyslog.Options{
	...
	logger.Error("Failed to connect",
		gooeyslog.Err(errors.New("connection refused")),
		gooeyslog.Red(slog.String("host", "db.example.com")),
	)

	// Using convenience color functions
	logger.Info("User action",
		gooeyslog.Green(slog.String("action", "login")),
		gooeyslog.Blue(slog.String("user", "alice")),
		gooeyslog.Cyan(slog.Int("session_id", 12345)),
	)
	...
	// Custom time format
	customLogger := slog.New(gooeyslog.NewHandler(os.Stderr, &gooeyslog.Options{
		TimeFormat: time.RFC3339,
	}))
	customLogger.Info("With RFC3339 timestamp")

	// With source location
	sourceLogger := slog.New(gooeyslog.NewHandler(os.Stderr, &gooeyslog.Options{
		AddSource: true,
	}))
	sourceLogger.Info("Shows file and line number")

	// Remove time from output using ReplaceAttr
	noTimeLogger := slog.New(gooeyslog.NewHandler(os.Stderr, &gooeyslog.Options{
	...
	slog.SetDefault(slog.New(gooeyslog.NewHandler(os.Stderr, gooeyslog.DefaultOptions())))
