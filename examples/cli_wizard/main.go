...
func main() {
	...
	app.Command("setup", "Step-by-step setup using prompts").
		Run(func(ctx *cli.Context) error {
			...
			switch language {
			case "go":
				frameworks = []string{"wonton", "gin", "echo", "fiber"}
			case "python":
		...
		Run(func(ctx *cli.Context) error {
			...
			editor := &ConfigEditor{
				items: []configItem{
					...
					{key: "framework", value: "wonton", options: []string{"wonton", "gin", "echo"}},
...
func (app *WizardApp) nextStep() {
	...
	case stepFramework:
		if app.config.Framework == "" {
			app.config.Framework = "wonton" // Default
		}
		app.step++
	case stepDatabase:
